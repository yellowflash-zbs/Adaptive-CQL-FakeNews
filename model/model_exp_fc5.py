...
#             self.article_source_embedding = nn.Embedding(len(self.article_source_dict), self.source_dim, padding_idx=0)
    ...
    # def forward(self, claim_tensors, just_tensors, src_tensors, oracle_ids, labels=None, lm_ids_dict=None):
        ...
        for _src in src_ids:
            # ======== 【修改开始：添加截断与无梯度保护，防止 OOM】 ========
            MAX_SENTENCES = 60  # 限制候选句子数量：最多只让前60句话进入BERT
            _src_truncated = _src[:MAX_SENTENCES]
            
            with torch.no_grad(): # 告诉显卡：这里只提取特征，不要保留海量的求导计算图
                src_repr.append([self.bert_embedding(s.to(self.device)).last_hidden_state[:,0,:] for s in _src_truncated])
            # ======== 【修改结束】 ========
